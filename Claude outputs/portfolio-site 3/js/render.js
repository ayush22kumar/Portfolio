...
/* ---------- Career & education timeline (home page) ----------
   Horizontal, interactive timeline over `timeline` (js/data.js). Nodes are
   real <button> elements in a single ARIA tablist; selecting one (by click,
   arrow keys, or the prev/next controls) swaps the detail panel below and
   scrolls the active node into view. Respects prefers-reduced-motion via
   the `prefersReducedMotion` flag defined above. */
function initTimeline() {
  const track = qs("#timeline-track");
  const panel = qs("#timeline-panel");
  const prevBtn = qs("#timeline-prev");
  const nextBtn = qs("#timeline-next");
  if (!track || !panel || typeof timeline === "undefined" || timeline.length === 0) return;
  ...
  timeline.forEach((item, i) => {
    const node = el("button", {
      class: "timeline-node" + (item.current ? " timeline-node--current" : ""),
    });
    node.type = "button";
    node.setAttribute("role", "tab");
    node.id = `timeline-tab-${item.id}`;
    node.setAttribute("aria-controls", "timeline-panel");
    node.setAttribute("aria-selected", "false");
    node.dataset.index = String(i);
    node.tabIndex = -1;

    node.appendChild(el("span", { class: "timeline-node-date", text: item.shortLabel }));
    const dotWrap = el("span", { class: "timeline-node-dot-wrap" });
    dotWrap.appendChild(el("span", { class: "timeline-node-dot" }));
  ...
  function setActive(index, opts = {}) {
    ...
    if (nextBtn) nextBtn.disabled = index === timeline.length - 1;
    ...
    if (opts.focus) nodes[index].focus();
  }

  track.addEventListener("click", (e) => {
    const node = e.target.closest(".timeline-node");
    if (!node) return;
    setActive(Number(node.dataset.index));
  });
  ...
  if (prevBtn) prevBtn.addEventListener("click", () => setActive(activeIndex - 1, { focus: true }));
  if (nextBtn) nextBtn.addEventListener("click", () => setActive(activeIndex + 1, { focus: true }));

  // Desktop drag-to-scroll. Touch devices already get native swipe scrolling
  // on the overflow-x:auto track, so this only binds pointer events for
  // mouse/pen input (a real touch drag would otherwise fight page scroll).
  let dragging = false;
  let dragStartX = 0;
