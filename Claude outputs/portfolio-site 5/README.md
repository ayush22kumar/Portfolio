...
- **Experience & education:** edit the `timeline` array, one object per
  milestone (oldest first). Each entry can carry an optional `related`
  sub-entry for a project, internship, or award that sits alongside it
  (e.g. the Capgemini internship under the B-Tech, or Oxfam under the MSc).
  Rendered as the interactive horizontal timeline on the homepage.
